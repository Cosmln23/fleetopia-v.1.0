...
router.put('/agent/toggle', requireAuth, (_req, res) => {
  res.json({ success: true, currentStatus: { L0: true, L1: false, L2: false }, warnings: [] });
});

router.post('/cost-settings', requireAuth, (_req, res) => {
  res.json({ success: true, totalBaseCost: 0 });
});

router.get('/agent/suggestions', requireAuth, (_req, res) => {
  res.json({ suggestions: [], pagination: { total: 0, pages: 0 } });
});

router.post('/agent/feedback', requireAuth, (_req, res) => {
